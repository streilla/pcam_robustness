...
MAGENetwork.DP_RATE[] = 0.1 #DROPOUT_RATE
...
py_train_transforms = torchvision.Compose(
    [
        ...
        # torchvision.RandomErasing(p = 0.5, scale = (0.02, 0.2)),
...
ma_tail = modelArchitecture(
    ...
    [Float64 for i in 1:L1],
    [4 for i in 1:L1]
)
model_args_tail = MAGENetwork._args_needed_per_model(MAGENetwork.ImagesToScalarNN, ma_tail, pylipbackend)
...
model_args_head = MAGENetwork._args_needed_per_model(MAGENetwork.ScalarsToScalarNN, ma_head, pylipbackend)

tail = MAGENetwork.create_nn_model(pylipbackend, MAGENetwork.ImagesToScalarNN, model_args_tail...; size = :gumbel_softmax, last = false)
head = MAGENetwork.create_nn_model(pylipbackend, MAGENetwork.ScalarsToScalarNN, model_args_head...; size = :large, last = true)
...
ImageDatasetClass = pylipext.get_ImageDataset()
ImageDataLoaderClass = pylipext.get_ImageDataLoader()
TrainDataset = ImageDatasetClass(
    map(i -> reduce((x, y) -> cat(x, y, dims = 3), map(x -> reinterpret.(UInt8, reinterpret(x.img)), i)), trainx),
    trainy .- 1, # vec of int
    py_train_transforms
)
ValDataset = ImageDatasetClass(
    map(i -> reduce((x, y) -> cat(x, y, dims = 3), map(x -> reinterpret.(UInt8, reinterpret(x.img)), i)), valx),
    valy .- 1, # vec of int
    py_val_transforms
)

TrainDataLoader = ImageDataLoaderClass(
    TrainDataset,
    batch_size = 128,
    shuffle = true,
    num_workers = Int(nthreads() / 2),
    pin_memory = true,
    prefetch_factor = 16,
    persistent_workers = true,
)
ValDataLoader = ImageDataLoaderClass(
    ValDataset,
    batch_size = 128,
    shuffle = false,
    num_workers = Int(nthreads() / 2),
...
outs = MAGENetwork.train_tail_and_heads(
    pylipbackend, m,
    tail, head,
...
tail, head = outs[1]
gumbel_model = pylipext._get_gumbelchain_layer(pylipbackend, tail, head)
...
# CAPTURE TRAIN
captured_train_data, idx_train = MAGENetwork.capture_surrogate_training_data(pylipbackend, gumbel_model, prepared_train_batches)
captured_val_data, idx_val = MAGENetwork.capture_surrogate_training_data(
    pylipbackend, gumbel_model, prepared_val_batches
)
...
@pyeval (w = outs[2].best_weights[1], filename = joinpath(folder, "model_weights_tail(1).pt")) => "torch.save(w, filename)"
@pyeval (w = outs[2].best_weights[2], filename = joinpath(folder, "model_weights_head(2).pt")) => "torch.save(w, filename)"
